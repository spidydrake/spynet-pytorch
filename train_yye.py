...
def train_one_epoch(dl: DataLoader,
    ...
    for i, (xf, xb, y) in enumerate(dl):
        x = xf.to(device), xb.to(device)
        y = y.to(device)
...
def build_dl(train_ds: Subset,
    ...
    for i, batch in enumerate(train_dl):
        print(f"Batch {i}: {len(batch[0])} samples")  # Assumes batch[0] returns the tuple of frame stacks
        if i == 2:  # Check first three batches and break
            break
...
def build_spynets(k: int, name: str,
    ...
    if name != 'none':
        ...
        current_train = pretrained.basic_module[k]
    else:
...
def train_one_level(k: int,
    ...
    if epoch % 100 == 0: 
        torch.save(current_level.state_dict(),
                str(Path(kwargs['checkpoint_dir']) / f'{k}_epoch_{epoch}.pt'))
...
if __name__ == "__main__":
    ...
    learning_rates = [1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2]  # Adjust these values as needed
